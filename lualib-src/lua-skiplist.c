...
# define DLOG(fmt, ...) \
  fprintf(stderr, "<lua-skiplist>" fmt "\n", ##__VA_ARGS__)
#else
# define DLOG(...)
#endif
...
static int compInLua(slNode_t *nodeA, slNode_t *nodeB, sl_t *sl, void *ctx)
{
  ...
  if (!lua_isfunction(L, -1)) {
    ...
    return luaL_error(L,
        "function not found! type=%s, value=%s,top=%d",
        typename, lua_tostring(L, -1), top);
  }
#endif
...
static int lua__new(lua_State *L)
{
  ...
  if (!lua_isfunction(L, 1) && !lua_isnil(L, 1) && !lua_isboolean(L, 1)) {
    luaL_argcheck(L, 0, 1,
        "compare function|boolean<true for desc, false or nil for asec>");
    return 0;
  }
...
static int lua__update(lua_State *L)
{
  ...
  if (lua_isnoneornil(L, 3) && node != NULL) {
    ...
    if (ret != 0) {
      return luaL_error(L,
          "compare function implementation maybe error in %s:%d",
