...
local exists, inspect, lfs

exists, inspect = pcall( require, "inspect" )
if not exists then inspect = nil end
exists, lfs = pcall( require, "lfs" )
if not exists then lfs = nil end
...
local function searchFilePath(modname)
  local middle = modname:gsub("%.", "/")
  local r
  for _, path in ipairs(filePaths) do
    local fileName = path[1] .. middle .. path[2]
    local file = io.open(fileName, "r")
    if file ~= nil then
      io.close(file)
      r = fileName
      filePathCache[modname] = fileName
      break
    end
  end
  return r
...
local function requireNew(modname)
  ...
  local checked = DoesFileExist(fileName)
  if not checked and searchFile then
    local realFileName = filePathCache[modname] or searchFilePath(modname)
    if realFileName then
      checked = true
      fileName = realFileName
    end
  end
  if checked and not fileCache[fileName] then
    local func, msg = loadfileNew(fileName)
    assert(func, msg)
...
function module.ReloadFile(fileName, ignoreTimestamp)
  ...
  local file = fileCache[fileName]
  print('ReloadFile============================', file, fileName)
...
function module.Reload(modname, ignoreTimestamp)
  local fileName = filePathCache[modname] or modname
  module.ReloadFile(fileName, ignoreTimestamp)
end
...
function module.Inject()
  ...
  for _, path in pairs(paths) do
    ...
    table.insert(filePaths, {left, right})
