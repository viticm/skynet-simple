...
local function getcfile(ctx, f)
  return string.format("%s/create/%s.%s", ctx.dir, ctx.name, f)  
end
...
function _M.load(ctx)
  ...
  ctx.tablecreate = setmetatable({}, {
    __index = function(t, k)
      local r = readfile(getcfile(ctx, "table." .. k))
      t[k] = r
      return r
    end
  })
...
function _M.save(ctx)
  ...
  for _, k in ipairs(ctx.table_list) do
    ...
    writefile(getcfile(ctx, "table." .. k), ctx.tablecreate[k])
  end
